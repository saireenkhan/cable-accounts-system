...
export default function DealerAreasPage() {
    ...
    try {
      ...
      if (dealersRes.data.success) {
        dealersRes.data.dealers.forEach((dealer: any) => {
          const areaName =
            typeof dealer.area === 'object' ? dealer.area?.name : dealer.area;
          if (areaName) {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Are you sure you want to delete dealer area "${name}"?`)) return;
    try {
  ...
  const handleEdit = (area: any, event?: React.MouseEvent) => {
      ...
      <div className="space-y-4">
        ...
        {filteredAreas.length === 0 ? (
          <div className="rounded-xl border border-dashed border-gray-300 bg-white py-16 text-center dark:border-gray-700 dark:bg-gray-800">
            ...
            <p className="text-sm font-medium text-gray-700 dark:text-gray-200">No dealer areas found</p>
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Try changing your search or filter.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {filteredAreas.map((area) => {
              const colors = colorMap[area.color as AreaColor];
                ...
                >
                  <div className="p-4">
                    ...
                    <div className="mt-4 flex items-center gap-2 border-t border-gray-100 pt-4 dark:border-gray-700">
                      <Truck className="h-4 w-4 flex-shrink-0 text-gray-500 dark:text-gray-400" />
                      <span className="text-xl font-bold text-gray-900 dark:text-white">
                        {(area.dealers || 0).toLocaleString()}
                      </span>
        ...
        <AddUserModal
          ...
          title={editingArea ? 'Edit Dealer Area' : 'Add New Dealer Area'}
          subtitle={editingArea ? 'Update the dealer area details below' : 'Create a new dealer service area'}
          fields={areaFields}
          submitLabel={editingArea ? 'Update Dealer Area' : 'Add Dealer Area'}
          color="blue"
          endpoint={editingArea ? `/dealer-areas/${editingArea.id}` : '/dealer-areas'}
